...
def verify_by_arxiv_id(arxiv_id: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ScholarClaw/0.1"})
...
def verify_by_doi(doi: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": "ScholarClaw/0.1 (mailto:scholarclaw_engine@example.com)",
                "Accept": "application/json",
            },
        )
...
_OPENALEX_EMAIL = "scholarclaw_engine@users.noreply.github.com"
...
def verify_by_openalex(title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": f"ScholarClaw/0.1 (mailto:{_OPENALEX_EMAIL})",
                "Accept": "application/json",
            },
        )
...
_CACHE_DIR = Path.home() / ".cache" / "scholarclaw_engine" / "citation_verify"
