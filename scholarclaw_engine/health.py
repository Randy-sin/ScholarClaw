...
from scholarclaw_engine.config import RCConfig, validate_config
...
def check_docker_runtime(config: RCConfig) -> CheckResult:
    ...
    if not DockerSandbox.ensure_image(docker_cfg.image):
        return CheckResult(
            ...
            fix=f"docker build -t {docker_cfg.image} scholarclaw_engine/docker/",
        )
...
def print_doctor_report(report: DoctorReport) -> None:
    ...
    print(f"ScholarClaw Doctor Report ({report.timestamp})")
