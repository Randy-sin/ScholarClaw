...
from scholarclaw_engine.hardware import HardwareProfile, detect_hardware, ensure_torch_available, is_metric_name
from scholarclaw_engine.llm import create_llm_client
...
from scholarclaw_engine.pipeline.contracts import CONTRACTS, StageContract
from scholarclaw_engine.experiment.validator import (
    CodeValidation,
...
def _get_evolution_overlay(run_dir: Path | None, stage_name: str) -> str:
    ...
    if run_dir is None:
        return ""
    try:
        from scholarclaw_engine.evolution import EvolutionStore
...
def _collect_experiment_results(
    ...
    from scholarclaw_engine.experiment.sandbox import extract_paired_comparisons
...
def _execute_literature_collect(
    ...
    *,
    ...
    try:
        from scholarclaw_engine.data import load_seminal_papers
...
def _multi_perspective_generate(
    ...
    *perspectives_dir* and returned as ``{role_name: response_text}``.
    """
    from scholarclaw_engine.prompts import _render  # noqa: PLC0415
...
def _execute_hypothesis_gen(
    ...
    *,
    ...
    if llm is not None:
        _pm = prompts or PromptManager()
        from scholarclaw_engine.prompts import DEBATE_ROLES_HYPOTHESIS  # noqa: PLC0415
    ...
    try:
        from scholarclaw_engine.literature.novelty import check_novelty  # noqa: PLC0415
...
def _execute_experiment_design(
    ...
    *,
    ...
    if llm is not None:
        ...
        try:
            from scholarclaw_engine.data import detect_frameworks, load_framework_docs
    ...
    if (
        ...
        try:
            from scholarclaw_engine.agents.benchmark_agent import BenchmarkOrchestrator
    ...
    *,
    ...
    if _bp_path is not None:
        try:
            ...
            from scholarclaw_engine.agents.benchmark_agent.orchestrator import BenchmarkPlan
            _bp = BenchmarkPlan(
    ...
    if config.experiment.code_agent.enabled and llm is not None:
        ...
        from scholarclaw_engine.pipeline.code_agent import CodeAgent as _CodeAgent

        _ca_cfg = config.experiment.code_agent
        # Ensure we have a proper config object
        if not hasattr(_ca_cfg, "enabled"):
            from scholarclaw_engine.pipeline.code_agent import (
        ...
        if config.experiment.mode in ("sandbox", "docker"):
            from scholarclaw_engine.experiment.factory import (
...
def _execute_experiment_run(
    ...
    *,
    ...
    from scholarclaw_engine.experiment.runner import ExperimentRunner
...
def _execute_iterative_refine(
    ...
    *,
    ...
    from scholarclaw_engine.experiment.factory import create_sandbox
    from scholarclaw_engine.experiment.validator import format_issues_for_llm, validate_code
        ...
        if validation.ok and config.experiment.mode in ("sandbox", "docker"):
            ...
            if rerun.timed_out:
                ...
                if not rerun.metrics and rerun.stdout:
                    from scholarclaw_engine.experiment.sandbox import parse_metrics as _parse_sb_metrics
...
def _execute_result_analysis(
    ...
    *,
    ...
    from scholarclaw_engine.experiment.sandbox import extract_paired_comparisons as _extract_paired
    ...
    if llm is not None:
        _pm = prompts or PromptManager()
        from scholarclaw_engine.prompts import DEBATE_ROLES_ANALYSIS  # noqa: PLC0415
...
"""
    ...
    if config.experiment.figure_agent.enabled and llm is not None:
        try:
            from scholarclaw_engine.agents.figure_agent import FigureOrchestrator
            from scholarclaw_engine.agents.figure_agent.orchestrator import FigureAgentConfig as _FACfg
...
def _validate_draft_quality(
    ...
    *stage_dir*.
    """
    from scholarclaw_engine.prompts import SECTION_WORD_TARGETS, _SECTION_TARGET_ALIASES
    ...
    *,
    ...
    if all_simulated:
        ...
        (stage_dir / "paper_draft.md").write_text(
            ...
            "config.scholarclaw.yaml and re-run the pipeline.",
            encoding="utf-8",
        )
        return StageResult(
            stage=Stage.PAPER_WRITE,
...
def _execute_export_publish(
    ...
    *,
    ...
    try:
        from scholarclaw_engine.templates import get_template, markdown_to_latex
        ...
        try:
            from scholarclaw_engine.templates.compiler import compile_latex
            ...
            if _compile_result.success:
                ...
                try:
                    from scholarclaw_engine.templates.compiler import check_compiled_quality
    ...
    try:
        ...
        from scholarclaw_engine.experiment.visualize import generate_all_charts
...
def _execute_citation_verify(
    ...
    *,
    ...
    from scholarclaw_engine.literature.verify import parse_bibtex_entries
        ...
        if (
            ...
            if mc_prm and getattr(mc_prm, "enabled", False):
                ...
                if int(stage) in prm_stages:
                    from scholarclaw_engine.metaclaw_bridge.prm_gate import ResearchPRMGate
