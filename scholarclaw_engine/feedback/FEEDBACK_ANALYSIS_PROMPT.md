...
> ```
> 请读取 scholarclaw_engine/feedback/FEEDBACK_ANALYSIS_PROMPT.md，然后按照指示处理 feedback_inbox/ 目录下的所有测试反馈。
> ```
...
你是 ScholarClaw 项目的高级 QA 工程师和代码架构师。你需要分析来自不同学科领域测试者的反馈，与当前 Pipeline 代码进行对比，生成一份结构化的 **Bug 修复文档**。

---

## 背景

ScholarClaw 是一个 23 阶段的全自动学术研究 Pipeline（从选题到论文生成）。我们招募了来自不同学科的测试者运行 Pipeline，他们提交了运行反馈和交付物。你需要帮我把这些反馈转化为可执行的 Bug 修复方案。
...
- `scholarclaw_engine/pipeline/stages.py` — 23 阶段定义和状态机
- `scholarclaw_engine/pipeline/executor.py` — 核心执行逻辑（重点关注各阶段的 execute 函数）
- `scholarclaw_engine/pipeline/runner.py` — Pipeline 运行入口
- `scholarclaw_engine/config.py` — 配置结构
- `scholarclaw_engine/llm/client.py` — LLM 调用逻辑
- `scholarclaw_engine/literature/search.py` — 文献搜索
- `scholarclaw_engine/experiment/docker_sandbox.py` — Docker 沙箱执行
- `scholarclaw_engine/pipeline/code_agent.py` — 代码生成 Agent
- `scholarclaw_engine/templates/converter.py` — LaTeX 转换
- `scholarclaw_engine/prompts.py` — Prompt 模板
