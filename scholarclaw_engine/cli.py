...
from scholarclaw_engine.health import print_doctor_report, run_doctor, write_doctor_report
...
def _resolve_config_or_exit(args: argparse.Namespace) -> Path | None:
    ...
    if path is None:
        ...
        print(
            ...
            f"Run 'scholarclaw_engine init' to create one from the example template.",
            file=sys.stderr,
        )
        return None
    return path
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    from scholarclaw_engine.pipeline.runner import execute_pipeline, read_checkpoint
    ...
    print(f"ScholarClaw v0.1.0 — Starting pipeline")
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    dest = Path("config.scholarclaw.yaml")
    ...
    if provider == "acp":
        ...
        print("  2. Edit config.scholarclaw.yaml to set llm.acp.agent if needed")
        print("  3. Run: scholarclaw_engine doctor")
    else:
        env_var = api_key_env or "OPENAI_API_KEY"
        print(f"\nNext steps:")
        print(f"  1. Export your API key: export {env_var}=sk-...")
        print("  2. Edit config.scholarclaw.yaml to customize your settings")
        print("  3. Run: scholarclaw_engine doctor")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="scholarclaw_engine",
        description="ScholarClaw — Autonomous Research Pipeline",
    )
    ...
    _ = run_p.add_argument(
        ...
        help="Config file (default: auto-detect config.scholarclaw.yaml or config.yaml)",
    )
    ...
    init_p = sub.add_parser("init", help="Create config.scholarclaw.yaml from example template")
    _ = init_p.add_argument(
        "--force", action="store_true", help="Overwrite existing config.scholarclaw.yaml"
    )
