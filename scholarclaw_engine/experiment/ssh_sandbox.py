...
from scholarclaw_engine.config import SshRemoteConfig
from scholarclaw_engine.experiment.sandbox import SandboxResult, parse_metrics
...
class SshRemoteSandbox:
    ...
    def check_ssh_available(config: SshRemoteConfig) -> tuple[bool, str]:
        ...
        cmd.append("echo scholarclaw_engine-ssh-ok")
        try:
            cp = subprocess.run(
                cmd, capture_output=True, text=True, timeout=15, check=False,
            )
            if cp.returncode == 0 and "scholarclaw_engine-ssh-ok" in cp.stdout:
