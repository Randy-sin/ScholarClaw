...
from scholarclaw_engine.experiment.sandbox import ExperimentSandbox, SandboxProtocol
...
def create_sandbox(config: ExperimentConfig, workdir: Path) -> SandboxProtocol:
    ...
    if config.mode == "docker":
        ...
        if not DockerSandbox.ensure_image(docker_cfg.image):
            raise RuntimeError(
                ...
                f"Build it: docker build -t {docker_cfg.image} scholarclaw_engine/docker/"
            )
    ...
    if config.mode == "ssh_remote":
        from scholarclaw_engine.experiment.ssh_sandbox import SshRemoteSandbox
