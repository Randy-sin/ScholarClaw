...
def main() -> None:
    ...
    image_ok = DockerSandbox.ensure_image("scholarclaw_engine/experiment:latest")
    check("Image exists locally", image_ok)
    if not image_ok:
        print("\nImage not found. Build it first:")
        print("  docker build -t scholarclaw_engine/experiment:latest scholarclaw_engine/docker/")
        sys.exit(1)
