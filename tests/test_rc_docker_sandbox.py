...
from scholarclaw_engine.config import DockerSandboxConfig, ExperimentConfig
from scholarclaw_engine.experiment.docker_sandbox import DockerSandbox
...
def test_harness_injection(tmp_path: Path):
    harness_src = Path(__file__).parent.parent / "scholarclaw_engine" / "experiment" / "harness_template.py"
...
def test_factory_returns_experiment_sandbox(tmp_path: Path):
    from scholarclaw_engine.experiment.sandbox import ExperimentSandbox
...
@patch("scholarclaw_engine.experiment.docker_sandbox.DockerSandbox.ensure_image", return_value=True)
@patch("scholarclaw_engine.experiment.docker_sandbox.DockerSandbox.check_docker_available", return_value=True)
...
@patch("scholarclaw_engine.experiment.docker_sandbox.DockerSandbox.check_docker_available", return_value=False)
...
@patch("scholarclaw_engine.experiment.docker_sandbox.DockerSandbox.ensure_image", return_value=False)
@patch("scholarclaw_engine.experiment.docker_sandbox.DockerSandbox.check_docker_available", return_value=True)
...
def test_ensure_image_true(mock_run):
    mock_run.return_value = subprocess.CompletedProcess(args=[], returncode=0)
    assert DockerSandbox.ensure_image("scholarclaw_engine/experiment:latest") is True
