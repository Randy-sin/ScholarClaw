...
from scholarclaw_engine import cli as rc_cli
from scholarclaw_engine.config import resolve_config_path
...
def test_main_with_no_args_returns_zero_and_prints_help(
    ...
    assert "ScholarClaw" in captured.out
...
def test_resolve_config_finds_arc_yaml_first(
    ...
    (tmp_path / "config.scholarclaw.yaml").write_text("x: 1\n")
    (tmp_path / "config.yaml").write_text("x: 2\n")
    result = resolve_config_path(None)
    assert result is not None
    assert result.name == "config.scholarclaw.yaml"
...
def test_cmd_init_creates_config(
    ...
    monkeypatch.chdir(tmp_path)
    _write_example_config(tmp_path / "config.scholarclaw.example.yaml")
    ...
    assert code == 0
    created = tmp_path / "config.scholarclaw.yaml"
    assert created.exists()
    content = created.read_text()
    assert 'provider: "openai"' in content
    assert "Created config.scholarclaw.yaml" in capsys.readouterr().out
...
def test_cmd_init_refuses_overwrite(
    ...
    monkeypatch.chdir(tmp_path)
    _write_example_config(tmp_path / "config.scholarclaw.example.yaml")
    (tmp_path / "config.scholarclaw.yaml").write_text("existing\n")
    args = argparse.Namespace(force=False)
    code = rc_cli.cmd_init(args)
    assert code == 1
    assert "already exists" in capsys.readouterr().err
    assert (tmp_path / "config.scholarclaw.yaml").read_text() == "existing\n"
...
def test_cmd_init_force_overwrites(
    ...
    monkeypatch.chdir(tmp_path)
    _write_example_config(tmp_path / "config.scholarclaw.example.yaml")
    (tmp_path / "config.scholarclaw.yaml").write_text("old\n")
    monkeypatch.setattr("sys.stdin", type("FakeStdin", (), {"isatty": lambda self: False})())
    args = argparse.Namespace(force=True)
    code = rc_cli.cmd_init(args)
    assert code == 0
    assert (tmp_path / "config.scholarclaw.yaml").read_text() != "old\n"
...
def test_cmd_run_missing_config_shows_init_hint(
    ...
    code = rc_cli.cmd_run(args)
    assert code == 1
    assert "scholarclaw_engine init" in capsys.readouterr().err
