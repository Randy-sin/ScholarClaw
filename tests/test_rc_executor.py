...
from scholarclaw_engine.pipeline import executor as rc_executor
...
def test_execute_stage_creates_stage_dir_writes_artifacts_and_meta(
    ...
    monkeypatch.setattr(
        "scholarclaw_engine.pipeline.executor.LLMClient.from_rc_config",
...
class TestStageHealth:
    def test_stage_health_json_written(self, tmp_path: Path) -> None:
        from scholarclaw_engine.pipeline.executor import execute_stage
        from scholarclaw_engine.pipeline.stages import Stage

        config = RCConfig.load(
            Path(__file__).parent.parent / "config.scholarclaw.example.yaml",
            check_paths=False,
        )
    ...
    def test_stage_health_has_required_fields(self, tmp_path: Path) -> None:
        ...
        with patch("scholarclaw_engine.pipeline.executor.LLMClient") as mock_llm_cls:
...
class TestParseMetricsFromStdout:
    ...
    def test_parses_simple_name_value(self) -> None:
        from scholarclaw_engine.pipeline.executor import _parse_metrics_from_stdout
...
class TestNaNDivergenceDetection:
    ...
    def test_parse_metrics_filters_nan(self) -> None:
        from scholarclaw_engine.experiment.sandbox import parse_metrics
    ...
    def test_detect_nan_divergence_finds_nan(self) -> None:
        from scholarclaw_engine.experiment.sandbox import detect_nan_divergence
...
class TestExperimentHarness:
    ...
    def test_harness_injected_into_sandbox(self, tmp_path: Path) -> None:
        import sys
        from scholarclaw_engine.config import SandboxConfig
