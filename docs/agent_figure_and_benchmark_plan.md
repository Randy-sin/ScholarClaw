...
> **Scope**: Two new multi-agent subsystems for ScholarClaw pipeline
>
...
> - prior work v2 and MLR-Copilot both use pure LLM-driven dataset selection (no API search) — our API-based approach is more structured
...
| 项目 | 图表策略 | 核心创新 |
|------|---------|---------|
| prior work v1 | 人工编写 `plot.py` 模板，LLM 不参与 | 可靠但不灵活 |
| prior work v2 | LLM 自主生成画图代码 + VLM 审查反馈循环 | **VLM-as-critic**，首篇通过 ICLR workshop 审稿 |
...
```yaml
# scholarclaw_engine/data/benchmark_knowledge.yaml
...
```python
# scholarclaw_engine/agents/base.py
...
两个系统都通过现有的 `scholarclaw_engine/llm/client.py` 调用 LLM：
...
| 来源 | 关键收获 |
|------|---------|
| [prior work v2](https://github.com/prior-group/prior-work-v2) | VLM-as-critic, 首篇通过 ICLR workshop 审稿 |
