...
1. Share the GitHub repo URL with OpenClaw:
   ```
   https://github.com/openclaw/scholarclaw
   ```
2. OpenClaw reads `SCHOLARCLAW_AGENTS.md` and `README.md` — it now understands the entire system.
   > **Note:** `SCHOLARCLAW_AGENTS.md` is generated locally and listed in `.gitignore`. If it doesn't exist, OpenClaw can bootstrap from `README.md` and the project structure.
...
scholarclaw run --config config.yaml --auto-approve

# Override topic from command line
scholarclaw run --config config.yaml --topic "Transformer attention for time series" --auto-approve
```
...
scholarclaw run -c config.yaml -t "Graph neural networks for protein folding" --auto-approve

# Resume a failed run from where it stopped
scholarclaw run -c config.yaml --resume --auto-approve

# Re-run just the paper writing stages
scholarclaw run -c config.yaml --from-stage PAPER_OUTLINE --auto-approve

# Check your setup before running
scholarclaw_engine doctor -c config.yaml
```
...
ScholarClaw supports four modes for running experiments:
...
```bash
docker build -t scholarclaw_engine/experiment:latest scholarclaw_engine/docker/
```
...
ScholarClaw generates LaTeX files formatted for specific conferences:
...
For deeper integration with OpenClaw, ScholarClaw includes a bridge adapter system. Each flag in the config activates a typed protocol interface:
...
[MetaClaw](https://github.com/openclaw/MetaClaw) adds **cross-run knowledge transfer** to ScholarClaw. When enabled, the pipeline automatically captures lessons from failures and converts them into reusable skills that improve subsequent runs.
...
git clone https://github.com/openclaw/MetaClaw.git
cd metaclaw && pip install -e .
```

#### Step 2: Configure

Add the `metaclaw_bridge` section to your `config.scholarclaw.yaml`:
...
| File | Purpose |
|------|---------|
| `scholarclaw_engine/metaclaw_bridge/` | Integration module (config, session, lesson_to_skill, prm_gate, skill_feedback) |
| `scholarclaw_engine/evolution.py` | `build_overlay()` — reads intra-run lessons + cross-run arc-* skills |
| `scholarclaw_engine/llm/client.py` | Proxy routing with automatic fallback |
...
ScholarClaw works with any AI coding assistant that can read project context files.

### Claude Code

Claude Code automatically reads `SCHOLARCLAW_CLAUDE.md` (if present) when you open the project. It also loads the skill definition from `.claude/skills/scholarclaw_engine/SKILL.md`.

> **Note:** `SCHOLARCLAW_CLAUDE.md` is generated locally and listed in `.gitignore`. The `.claude/skills/scholarclaw_engine/SKILL.md` file is always available in the repo.
...
OpenCode loads skills from `.claude/skills/`. The `scholarclaw_engine` skill activates on research-related queries and guides the agent through the pipeline.

### Any AI CLI

Provide `SCHOLARCLAW_AGENTS.md` (if generated locally) or `README.md` as context to any AI assistant. `SCHOLARCLAW_AGENTS.md` contains:
- The agent role definition (research orchestrator)
- Quick setup instructions
- Pipeline stage reference
- Decision guide for common scenarios

The agent reads this file and knows how to install, configure, and run the pipeline. If the file is not present, the `README.md` and `.claude/skills/scholarclaw_engine/SKILL.md` provide sufficient context for any AI assistant to operate the pipeline.
...
scholarclaw_engine doctor --config config.yaml
```
...
| `Config validation FAILED` | Invalid YAML or missing fields | Run `scholarclaw validate -c config.yaml` for details |
...
scholarclaw run -c config.yaml --from-stage EXPERIMENT_RUN --auto-approve --output artifacts/<run-id>
```

### Reading a Run Report

```bash
scholarclaw_engine report --run-dir artifacts/rc-20260310-143200-a1b2c3
```
...
*Last updated: March 2026 · ScholarClaw v0.3.0*
