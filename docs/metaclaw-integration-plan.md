# MetaClaw × ScholarClaw 集成方案

> **Status**: ✅ **Implemented & Merged to main** (v0.3.0, 2026-03-16)
>
> **目标**: 将 MetaClaw 的持续学习能力（技能注入、技能进化、PRM 评分、RL 训练）接入 ScholarClaw 的 23 阶段研究流水线，提升端到端论文生成质量。
...
| 项目 | 定位 | 核心能力 |
|------|------|----------|
| **ScholarClaw** | 全自主研究流水线（Idea → Paper） | 23 阶段 Pipeline、文献检索、实验执行、论文写作、引用验证 |
| **MetaClaw** | Agent 持续进化平台 | 技能注入（Skill Injection）、技能进化（Skill Evolution）、PRM 奖励评分、RL 微调、空闲调度器 |

**集成核心思路**: MetaClaw 作为 ScholarClaw 的 **LLM 增强层**，通过多层次赋能提升每个阶段的 LLM 输出质量，并建立从研究失败中持续学习的闭环。
...
| 层次 | 名称 | 改动范围 | 效果 |
|------|------|----------|------|
| **L1** | Proxy 透传 | 仅改配置 | ScholarClaw → MetaClaw Proxy → LLM，自动获得通用技能注入 |
| **L2** | 阶段感知技能 | 新增研究技能库 + 阶段映射 | 每个 Pipeline 阶段注入最相关的研究技能 |
| **L3** | Evolution 桥接 | 新增 bridge 模块 | ScholarClaw 失败教训 → MetaClaw 技能；双向学习闭环 |
...
#### Task 0.1: 创建集成分支
```bash
cd /home/jqliu/projects/ScholarClaw
...
**目标**: 零代码改动，仅通过配置让 ScholarClaw 经由 MetaClaw 代理调用 LLM。

#### Task 1.1: 修改 ScholarClaw 配置

修改 `config.scholarclaw.yaml`:
```yaml
llm:
...
在 `scholarclaw_engine/llm/client.py` 中处理 MetaClaw 可能返回的 503 状态码（权重更新中）：

**文件**: `scholarclaw_engine/llm/client.py`
**改动**: 将 503 加入可重试状态码列表
...
# 2. 运行 ScholarClaw 短流程
scholarclaw run --topic "test topic" --config config.yaml
```

**验证点**:
- [x] ScholarClaw 能正常调用 LLM
- [x] MetaClaw 日志显示技能注入
- [x] 输出质量与直连 LLM 相当或更好

**预期交付**: ScholarClaw 透过 MetaClaw 运行，自动获得通用技能加持。

---

### Phase 2: L2 — 研究专属技能库 + 阶段映射

**目标**: 为 ScholarClaw 的 23 个阶段创建专属技能，并实现精准注入。
...
**新增文件**: `scholarclaw_engine/metaclaw_bridge/stage_skill_map.py`
...
修改 `scholarclaw_engine/llm/client.py`，在发送请求时附带阶段上下文 Header：
...
**目标**: 让 ScholarClaw 的失败教训自动转化为 MetaClaw 技能，形成学习闭环。

#### Task 3.1: Lesson → Skill 转化器

**新增文件**: `scholarclaw_engine/metaclaw_bridge/lesson_to_skill.py`
...
**新增文件**: `scholarclaw_engine/metaclaw_bridge/skill_feedback.py`
...
在 `scholarclaw_engine/pipeline/executor.py` 的运行结束钩子中，添加:
...
async def _post_pipeline_hook(self, run_results: list[StageResult]):
    ...
    if self.config.metaclaw_bridge.enabled:
        from scholarclaw_engine.metaclaw_bridge.lesson_to_skill import convert_lessons_to_skills
...
**新增文件**: `scholarclaw_engine/metaclaw_bridge/prm_gate.py`

**功能**: 封装 MetaClaw PRMScorer，为 ScholarClaw 的质量门控提供评分。
...
在 ScholarClaw 的 Pipeline runner 中管理 MetaClaw 会话:
...
| 文件 | 改动内容 |
|------|----------|
| `scholarclaw_engine/llm/client.py` | 添加 503 重试 + X-Session-Id/X-Turn-Type header |
| `scholarclaw_engine/config.py` | 新增 `metaclaw_bridge` 配置段 |
| `scholarclaw_engine/pipeline/executor.py` | 添加 post-pipeline hook 调用 lesson_to_skill |
| `config.scholarclaw.example.yaml` | 添加 metaclaw_bridge 配置示例 |
...
在 `scholarclaw_engine/llm/client.py` 中实现:
...
| API | 用途 | 是否必需 |
|-----|------|----------|
| OpenAI-compatible LLM API | ScholarClaw + MetaClaw 共用 | **必需** |
| PRM 评分用 LLM API | Phase 4 质量门控（可与上述相同） | Phase 4 需要 |
| Tinker/MinT API | Phase 5 RL 训练 | **可选** |
| arXiv API | ScholarClaw 文献检索（已有） | 已配置 |
| Semantic Scholar API | ScholarClaw 文献检索（已有） | 已配置 |
...
scholarclaw run \
  ...
  --config config.scholarclaw.yaml
