# ScholarClaw — Docker Sandbox Environment Enhancement Plan
...
**File:** `scholarclaw_engine/pipeline/executor.py:2062-2073`
**Current:**
```python
...
**File:** `scholarclaw_engine/experiment/docker_sandbox.py:169-181, 317-354`
...
**File:** `scholarclaw_engine/config.py:163`
...
**File:** `scholarclaw_engine/experiment/docker_sandbox.py` (no implementation exists)
...
**File:** `scholarclaw_engine/docker/Dockerfile:27-30` vs `scholarclaw_engine/prompts.py:328-332`
...
**File:** `scholarclaw_engine/docker/Dockerfile:20-24`
**Missing packages:**
...
**File:** `scholarclaw_engine/prompts.py:333`
...
**File:** `scholarclaw_engine/pipeline/executor.py` (code_generation stage)
...
**File:** `scholarclaw_engine/experiment/docker_sandbox.py`
...
**File:** `scholarclaw_engine/prompts.py`
...
**File:** `scholarclaw_engine/pipeline/executor.py:2062`
...
**Files:**
- `scholarclaw_engine/docker/entrypoint.sh` (NEW) — wrapper script
- `scholarclaw_engine/docker/Dockerfile` (MODIFY) — new entrypoint
- `scholarclaw_engine/experiment/docker_sandbox.py` (MODIFY) — refactor execution model

**E2.1: Create wrapper entrypoint script** (`scholarclaw_engine/docker/entrypoint.sh`)
```bash
#!/bin/bash
set -e
...
**Files:**
- `scholarclaw_engine/prompts.py` — add `setup_script_guidance` block
- `scholarclaw_engine/pipeline/executor.py` — code generation stage generates setup.py alongside main.py
...
**File:** `scholarclaw_engine/docker/Dockerfile:27-30`
...
**File:** `scholarclaw_engine/docker/Dockerfile`
...
**File:** `scholarclaw_engine/prompts.py:323-369`
...
**Files:**
- `scholarclaw_engine/prompts.py` — add requirement to code_generation prompt
- `scholarclaw_engine/experiment/docker_sandbox.py` — auto-generate from detected imports
...
**File:** `scholarclaw_engine/data/dataset_registry.yaml` (NEW)
...
| Action | File | Tasks |
|--------|------|-------|
| CREATE | `scholarclaw_engine/docker/entrypoint.sh` | E2 |
| CREATE | `scholarclaw_engine/data/dataset_registry.yaml` | E9 |
| MODIFY | `scholarclaw_engine/docker/Dockerfile` | E2, E5, E6, E10 |
| MODIFY | `scholarclaw_engine/experiment/docker_sandbox.py` | E2, E3, E8 |
| MODIFY | `scholarclaw_engine/config.py` | E3 |
| MODIFY | `scholarclaw_engine/pipeline/executor.py` | E1, E4 |
| MODIFY | `scholarclaw_engine/prompts.py` | E4, E7 |
