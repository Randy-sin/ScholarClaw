...
> 本文档详细描述了为 ScholarClaw LLM 模块引入 Anthropic Messages API 原生支持的改动内容，
...
ScholarClaw 的 LLM 模块原先仅支持 **OpenAI Chat Completions API 格式**（含兼容此格式的 OpenRouter、DeepSeek 等）。
...
| 文件路径 | 变更类型 | 改动说明 |
|---|---|---|
| `scholarclaw_engine/llm/__init__.py` | 修改 | 添加 `"anthropic"` preset；简化工厂函数委托给 `from_rc_config()` |
| `scholarclaw_engine/llm/client.py` | 修改 | `from_rc_config()` 恢复 PRESETS 逻辑 + 条件挂载适配器；`_raw_call()` 添加 if/else 分支 |
| `scholarclaw_engine/llm/anthropic_adapter.py` | **新增** | `AnthropicAdapter` 类 — Anthropic Messages API → OpenAI 兼容格式转换 |
...
| 依赖 | 版本要求 | 安装方式 | 说明 |
|---|---|---|---|
| `httpx` | `>=0.24` | `pip install scholarclaw_engine[anthropic]` | **可选依赖**，仅 Anthropic provider 需要 |

不使用 Anthropic provider 的用户 **无需安装 httpx**，`pip install scholarclaw_engine` 的行为完全不变。
