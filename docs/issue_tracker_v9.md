...
- **Files**:
  - `scholarclaw_engine/pipeline/executor.py:240-253` — regex now matches `#{1,2}`, strips "Title " prefix
  - `scholarclaw_engine/templates/converter.py:429-451` — `_extract_title()` now handles level 1 and 2
...
- **File**: `scholarclaw_engine/templates/converter.py:434,442,447` — `sec.level in (1, 2)` + "Title " prefix strip
...
- **File**: `scholarclaw_engine/data/seminal_papers.yaml`
...
- **Proposed Approaches**:
  - **Option A: Static doc snippets** — Bundle curated API reference snippets for common frameworks in `scholarclaw_engine/data/framework_docs/`. Simple, fast, but requires manual updates.
...
| **prior work v2** | No templates, relies purely on LLM knowledge + tree search debugging | Zero setup | Lower success rate, no doc awareness |
...
**Key finding**: No existing autonomous research agent (prior work v1/v2, CodeScientist) dynamically reads documentation at runtime. They all rely on pre-built templates or LLM training data. **Doc-RAG would be a differentiating feature for ScholarClaw.**
...
- Create `scholarclaw_engine/data/framework_docs/` directory
...
- Add `FrameworkDocFetcher` class in `scholarclaw_engine/literature/framework_docs.py`
...
- Cache fetched docs locally (`.scholarclaw_cache/framework_docs/`)
...
- Created `scholarclaw_engine/data/framework_docs/` with 5 curated API reference files:
...
- Added `detect_frameworks()` and `load_framework_docs()` in `scholarclaw_engine/data/__init__.py`
...
**Design Patterns Identified** (from survey of 12+ systems: Claude Code, Cursor, Devin, SWE-Agent, OpenHands, Aider, MetaGPT, ChatDev, prior work v2, AIDE, AgentCoder, AlphaCodium):
...
| B: Solution Tree Search | Solutions as tree nodes; branch, evaluate, prune | prior work v2, AIDE | CRITICAL |
...
- Files: `scholarclaw_engine/pipeline/executor.py` (new substage), `scholarclaw_engine/prompts.py` (architecture prompt)
...
- Files: `scholarclaw_engine/pipeline/executor.py`, `scholarclaw_engine/experiment/docker_sandbox.py`
...
- Inspired by AIDE/prior work v2 tree search pattern
- Files: New `scholarclaw_engine/pipeline/code_agent.py`, `scholarclaw_engine/pipeline/executor.py`
...
- Files: `scholarclaw_engine/pipeline/code_agent.py`
...
|------|------------|-------|
| `scholarclaw_engine/pipeline/executor.py` | 14 | ~6000 |
| `scholarclaw_engine/prompts.py` | 10 | ~2500 |
| `scholarclaw_engine/templates/converter.py` | 5 | ~1200 |
| `scholarclaw_engine/experiment/docker_sandbox.py` | 3 | ~420 |
| `scholarclaw_engine/docker/Dockerfile` | 3 | ~45 |
