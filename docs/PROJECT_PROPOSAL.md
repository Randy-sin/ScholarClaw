...
| 板块 | 章节 | 内容 |
|------|------|------|
| **项目总览** | 第一章 | 项目概述——一段话说清 ScholarClaw 是什么，核心数据一览 |
| **背景与动机** | 第二章 | 解决的核心问题——学术写作痛点、现有工具不足、我们的方案 |
| **系统架构** | 第三章 | 技术架构——23 阶段流水线总览、门控回滚、阶段契约系统 |
| **核心技术** | 第四章 | 八大技术亮点——五源检索、四层引用验证、硬件感知、自修复沙箱、多智能体、自动评审、跨运行学习、缓存优化 |
| **质量保障** | 第五章 | 技术指标——1,117+ 测试用例、LLM 容错、安全机制 |
| **竞品对比** | 第六章 | 与 AI Scientist、ChatGPT 等同类项目的逐项差异化 |
| **应用场景** | 第七章 | 三大场景——科研加速、教育辅助、产业研发 |
| **技术栈** | 第八章 | 语言、框架、API、工具链全景 |
| **项目现状** | 第九章 | 当前版本 v0.5.0 功能清单 + 后续路线图 |
| **平台集成** | 第十章 | ScholarClaw 与 OpenClaw 的集成架构——技能发现、六端口适配器、论文导出、生态协作、ACP 协议 |
| **团队与开源** | 第十一章 | 开源协议、代码仓库、文档 |
| **总结与愿景** | 第十二章 | 我们做了什么、解决了什么、为什么属于 OpenClaw、更远的未来 |
...
ScholarClaw 将上述所有步骤**串联为一条 23 阶段的确定性流水线**，一次命令完成全流程：
...
系统采用**阶段化流水线架构**，23 个阶段按职责划分为 8 个阶段组：

```
┌─────────────────────────────────────────────────────────────────┐
│                                                                 │
│  A 研究定义                        E 实验执行                   │
│    1  主题初始化 + 硬件探测          12  沙箱执行                │
│    2  问题分解                       13  迭代优化 ← 自修复      │
│                                                                 │
│  B 文献发现                        F 分析与决策                 │
│    3  检索策略生成                   14  多视角结果分析          │
│    4  多源文献采集 ← 5个API         15  研究决策 ← PIVOT/REFINE│
│    5  文献筛选 [门控]                                           │
│    6  知识抽取                     G 论文撰写                   │
│                                     16  论文大纲                │
│  C 知识综合                         17  分段撰写初稿            │
│    7  发现聚类 + 空白识别            18  多Agent同行评审         │
│    8  假设生成 ← 多Agent辩论        19  带保障的修订            │
│                                                                 │
│  D 实验设计                        H 终稿输出                   │
│    9  实验方案设计 [门控]            20  质量门控 [门控]         │
│   10  硬件感知代码生成               21  知识归档                │
│   11  资源规划                       22  LaTeX导出 + 会议适配   │
│                                      23  四层引用验证           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
- **3 个门控阶段**（第 5、9、20 阶段）：支持人工审批介入，也可用 `--auto-approve` 自动通过。拒绝时自动回滚（5→4、9→8、20→16）。
- **2 个反馈循环**：第 15 阶段根据实验结果自主决策——PROCEED（继续）、REFINE（调参重跑，回到第 13 阶段）、PIVOT（假设不成立，回到第 8 阶段重新生成假设）。最多允许 2 次 PIVOT。
- **产物版本化**：每次 PIVOT/REFINE 自动快照之前的产物（`stage-08_v1/`、`stage-08_v2/`），确保决策演化完全可追溯。
...
**LLM 生成论文的最大风险是幻觉引用——编造不存在的论文。** ScholarClaw 在论文初稿完成后（第 23 阶段），对每一条引用进行四层独立验证：
...
ScholarClaw 不是生成一段"理论上能跑"的代码然后让用户自己解决环境问题。它在**第 1 阶段就探测本地硬件**，并据此调整后续所有阶段的代码生成策略：
...
论文初稿完成后（第 18 阶段），系统运行**结构化的多 Agent 评审**：
...
| 维度 | AI Scientist (Sakana AI) | ChatGPT/Claude 直接写 | ScholarClaw |
|------|------------------------|---------------------|-------------|
| 文献来源 | LLM 记忆 | LLM 记忆 | **5 个真实学术 API** |
| 引用验证 | 无 | 无 | **四层独立验证** |
| 实验执行 | 固定模板 | 不执行 | **5 种模式 + 自修复** |
| 硬件适配 | 无 | 无 | **自动探测 GPU/MPS/CPU** |
| 反馈循环 | 无 | 无 | **PIVOT/REFINE 自主决策** |
| 会议模板 | 有限 | 无 | **6 套会议模板** |
| 同行评审 | 无 | 无 | **多 Agent 结构化评审** |
| 跨运行学习 | 无 | 无 | **Evolution + MetaClaw** |
| 测试覆盖 | 未公开 | N/A | **1,117+ 测试用例** |
...
我们用 23 个阶段、5 个学术 API、四层引用验证、五模式实验沙箱和三个专用智能体，构建了一条从"一句话"到"一篇可编译论文"的完整流水线。这不是一个玩具 demo——它有 1,117 个测试用例、200+ 可配参数、完整的错误恢复机制和跨运行的自学习能力。
...
| 各环节之间靠人工衔接 | 23 阶段确定性流水线，带门控、回滚和反馈循环 |
